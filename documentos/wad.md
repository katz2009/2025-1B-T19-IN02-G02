...
1. O agente de saúde (AGENTE), em contato com o paciente, preenche (PREENCHE) o histórico do paciente (HISTÓRICO), que consiste em informações básicas deste, com o objetivo de tornar os preenchimentos subsequentes mais concisos e claros. Esse HISTÓRICO também é elaborado com base no "Formulário 1" (FORMULARIO1), que é um formulário subjetivo preenchido com as percepções do paciente, configurando uma relação de 1 para 1 (um paciente preenche apenas um formulário deste tipo, que permanece em seu histórico).
